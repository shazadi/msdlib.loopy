...
The HOPP Driver Generator can be called using a command line interface (CLI). The CLI offers several parameters to further configure the run of the generator, listed in \Cref{tab:cliParams1,tab:cliParams2}. Note, that individual backends may provide additional parameters, that are not listed in this table. The CLI parameters are exclusively used to modify the behaviour of the driver generator. The behaviour of the driver itself is only influenced by the provided \texttt{.bdl} file.
...
On the board, out-going ports are represented by their software and hardware queues \texttt{q} and \texttt{r}, similar to in-going ports. The board representation is comparably simple (\Cref{fig:cfg:boardOut:fwd}), since no incoming messages have to be processed. Instead, values are simply shifted to the software queue as long as possible, i.e., until either the hardware queue is emptied or the software queue is filled. All collected values are then transmitted to the host-side driver.
...
\textcolor{red}{ReqCheck} & 0001 & Unused & Unused & No\\
Checksum & 0010 & Unused & Unused & No\\
...
This message is sent by the board-side driver to the host-side driver in response to the checksum request.The Message is specified in the protocol.After sending the checksum request ,decode function is  called and it constructs the header of the message and decides the message type. According to the type of the message, it is passed to the switch statement.If the message is a checksum then it is compared with the host side checksum values.The checksum values are defined in the constant header file.
 
On successful comparison,connection continues. The checksum matching shows that the board-side driver and the host-api are generated from the same description-file .Otherwise, a error message “Checksum of board-side driver and host-side api do not match” is displayed on the console. The error message is written in the protocol decode function and checksum_recvd returns the bool value for it. The checksum_rcvd value is accessed via proto->checksum_received function which is called in the check checksum function. If checksum doesn't match then checksum_received returns the checksum_rcvd false. After that exit call terminates the program. 
...
\subsubsection{Checksum Implementation}
Checksum implementation shows that the board-side driver and the host-api are generated from the same description-file The host-side and the board-side drivers have their own checksum values. Once the connection established, the host-side driver request for the board-side checksum by sending a checksum request. After sending a request, the host driver waits for the board-side checksum. The send checksum function gets the bool value for the checksum received. The value of this variable is returned by proto->checksum_received() called from the main-thread (through check_checksum()) and the reader-thread writes to this variable in protocol decode function.

In the protocol decode a switch statement has checksum case where checksum of both sides compared. The checksum values of both drivers compared sequentially. First value of the host-side driver with the first board-side checksum values then second of both and so on. If one of them not matched checksum not matched. If checksum matched program keeps continue and “Your” function called from the main where values are read and write through an out-going AXI-stream port and in-going AXI-stream port respectively. Host-side reads the values through an out-going port from the board-side driver in the “Your” function. 
...
% Why are there two different classes for model-specific helper methods... merge this or make a clearer distinction by moving all C/C++ model related methods into one file and all others in the second...
Similar, \texttt{de.hopp.generator.backends.workflow.ise.xps} provides such methods for the mhs model.
% Again - why is this so distributed? And why is the C/C++ model in the util package, while the MHS model is in the corresponding backend package?
% (possible issue: C/C++ backend can be used by both, ise and vivado flow... still, there are better ways to handle this... for example moving these to the model package)
