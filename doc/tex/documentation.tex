...
\subsection{Checksum Implementation}
The host-side and the board-side drivers have their own checksum values. Once the connection  has been established, the host-side driver requests for the board-side checksum by sending a checksum request. After sending a request, the host driver waits for the board-side checksum. The value of the “checksum rcvd” variable is returned by proto->checksum_received()  which is called from the main-thread (through check_checksum()) and then the  reader-thread writes to its variable in protocol decode function.

A switch statement ,inside the protocol decode function has different messages. They are Data, Poll, GPIO, Ack, Reset and Request checksum. Each message is passed to different cases. In checksum case, decode function receives the board-side checksum values and compares them with the host side checksum values. These values are compared sequentially- first value of the board-side with the  first value of host-side and so on. If one of the value fails to match, then Checksum do not match. Checksum implementation makes the connection more efficient and secure since the host-side driver doesn't send its data until it gets the result of the matched checksums.
